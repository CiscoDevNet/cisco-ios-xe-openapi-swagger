#!/usr/bin/env python3
"""
Generate OpenAPI 3.0 specs from resolved YANG tree HTML files for MIB modules.
MIB data is read-only (GET only), similar to operational data.
Parses each *-MIB.html tree to produce deep GET-only specs.
"""
...
# ---------------------------------------------------------------------------
# Tree parser (shared across all tree-based generators)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Path collection & operations (GET-only for MIB data)
# ---------------------------------------------------------------------------
...
def create_spec(title, description, tag, paths, module_name, version='17.18.1'):
    return {
        ...
        'info': {
            ...
            'x-yang-module': module_name, 'x-model-type': 'mib'
        },
        'servers': [{
...
def process_tree_file(html_path, output_dir, max_depth=5):
    ...
    for module_name, root_list in module_roots.items():
        ...
        title = f"Cisco IOS-XE MIB - {module_name}"
        desc = (f"MIB data from `{module_name}` module.\n\n"
...
def generate_all():
    ...
    output_dir = script_dir.parent / 'swagger-mib-model' / 'api-v2'
    ...
    print(f"\n{'='*70}")
    print("MIB YANG Tree -> OpenAPI 3.0 Generator (v2)")
    print(f"{'='*70}\n")

    # Find MIB tree files (contain -MIB in name or ETHER-WIS)
    tree_files = sorted([f for f in tree_dir.glob('*.html')
                         if '-MIB' in f.stem or 'MIB-' in f.stem or f.stem == 'ETHER-WIS'])
    print(f"Found {len(tree_files)} MIB tree files\n")
    ...
    manifest = {
        ...
        'generator': 'generate_mib_from_tree.py',
        'source': 'yang-trees/*-MIB.html',
        'version': '17.18.1'
    }
