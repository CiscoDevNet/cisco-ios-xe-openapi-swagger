...
# Module-level flag set by _parse_version_arg() to indicate whether
# ``--version`` was actually supplied on the command line. Needed because the
# parser strips the flag from ``sys.argv`` before resolve_paths inspects it.
_VERSION_FLAG_SUPPLIED = False
...
def _parse_version_arg() -> str:
    ...
    """
    global _VERSION_FLAG_SUPPLIED
    argv = sys.argv
    for i, tok in enumerate(argv):
        if tok == "--version" and i + 1 < len(argv):
            ver = argv[i + 1]
            del argv[i:i + 2]
            _VERSION_FLAG_SUPPLIED = True
            return ver
...
def resolve_paths(model_category: str, *, mib_subdir: bool = False) -> tuple[Path, Path, str]:
    ...
    if version == LEGACY_VERSION and not _VERSION_FLAG_SUPPLIED:
        ...
        # Explicit --version 17.18.1: read from the legacy YANG tree but
        # write into the per-release layout so the dataset is comparable
        # with newer releases generated by the same code.
        yang_dir = PROJECT_ROOT / "references" / "17181-YANG-modules"
        if mib_subdir:
            yang_dir = yang_dir / "MIBS"
