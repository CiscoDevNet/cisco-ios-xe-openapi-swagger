...
def build_index_for_root(root_dir):
    ...
    if not found_any:
        return None
    enrich_with_notifications(root_dir)
    categories = {
...
def enrich_with_notifications(root_dir):
    """Fold the cross-model notification index into the search keywords so a
    search for a notification name (e.g. ``cefResourceFailure``), the word
    ``notification``/``trap``, or the module itself surfaces the module and
    flags that it defines notifications. Generated by
    generators/generate_notifications_index.py; no-op if absent.
    """
    notif_path = os.path.join(root_dir, 'notifications.json')
    ...
    try:
        ...
        return
    by_module = {m['module']: m for m in notif.get('modules', [])}
    if not by_module:
        return
    for entry in modules:
        info = by_module.get(entry['name'])
        if not info:
            continue
        kw = set(entry.get('keywords', []))
        kw.add('notification')
        kw.add(info.get('transport', ''))
        if info.get('transport') == 'snmp-trap':
            kw.add('trap')
        for n in info.get('notifications', []):
            nm = (n.get('name') or '').lower()
            if nm:
                kw.add(nm)
        kw.discard('')
        entry['keywords'] = sorted(kw)[:80]
        entry['notificationCount'] = info.get('notification_count', 0)
