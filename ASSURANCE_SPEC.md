...
| G-6 | API / operation / module count regression guard | `python -X utf8 scripts/release_counts.py --check` (also covered by G-1 via `tests/test_release_counts.py`) | Any per-category drop in spec count, path count, operation count, search-index modules, or platform-support modules vs the checked-in [release_counts.json](release_counts.json) baseline — **block deploy**. Refresh baseline with `--write` only when the drop is intentional. Weekly auto-refresh PR via [`.github/workflows/g6-baseline-refresh.yml`](.github/workflows/g6-baseline-refresh.yml) (Mondays 06:00 UTC, manual `workflow_dispatch` available). |
